...
function playerLogin.onLogin(player)
	...
	local defaultTownId = TOWNS_LIST.THAIS -- default town where player is teleported if his home town is in premium area
	local freeTownIds = { TOWNS_LIST.AB_DENDRIEL, TOWNS_LIST.CARLIN, TOWNS_LIST.KAZORDOON, TOWNS_LIST.THAIS, TOWNS_LIST.VENORE, TOWNS_LIST.ROOKGAARD, TOWNS_LIST.DAWNPORT, TOWNS_LIST.DAWNPORT_TUTORIAL, TOWNS_LIST.ISLAND_OF_DESTINY }

	if not player:isPremium() and not table.contains(freeTownIds, player:getTown():getId()) then
		local town = player:getTown()
		local sex = player:getSex()
		local home = getHouseByPlayerGUID(getPlayerGUID(player))
		town = table.contains(freeTownIds, town:getId()) and town or Town(defaultTownId)
		if not town then
			print("[Login] ERROR: Default town ID '" .. defaultTownId .. "' not found.")
			return true
		end
